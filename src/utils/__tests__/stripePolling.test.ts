...
describe('stripe polling helpers', () => {
  it('completes when balance reaches baseline + expected amount', () => {
    expect(shouldCompleteStripePolling(1000, 200, 1200)).toBe(true);
  });

  it('ignores interim balance dips before mint', () => {
    expect(shouldCompleteStripePolling(1000, 100, 900)).toBe(false);
    expect(shouldCompleteStripePolling(1000, 100, 1100)).toBe(true);
  });
});
