...
describe('stripe checkout api', () => {
  ...
  it('posts Stripe checkout payload', async () => {
    mockPost.mockResolvedValueOnce({ data: { checkout_id: 1 } });

    await createStripeCheckout({ amountCents: 500, currency: 'USD' });

    expect(mockPost).toHaveBeenCalledWith('/payments/stripe/checkout/', {
      amount_cents: 500,
      currency: 'USD',
    });
  });
  ...
  it('maps auth errors', () => {
    ...
    const parsed = normalizeStripeApiError(error);
  ...
  it('maps field validation errors', () => {
    const error = createAxiosError(400, {
      currency: ['Currency not supported for Stripe.'],
    });

    const parsed = normalizeStripeApiError(error);

    expect(parsed.status).toBe(400);
    expect(parsed.message).toBe('Currency not supported for Stripe.');
    expect(parsed.fields?.currency?.[0]).toBe('Currency not supported for Stripe.');
  });
});
